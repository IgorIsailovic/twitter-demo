...
@Entity
@Table(name = "tweets", indexes = { @Index(name = "tweetIdIndex", columnList = "tweet_id"),
...
public class TweetResp {
	...
	// @Size(max=320, message = "Tweet body must not be greater than 320
	// characters!")
	private String tweetBody;

	@ElementCollection(fetch = FetchType.EAGER)
	@CollectionTable(name = "hash_tags", joinColumns = @JoinColumn(name = "tweet_id"), indexes = {
			@Index(name = "hashTagIndex", columnList = "hash_tags"), })
	@Column(name = "hash_tags")
	// @Size(max=5, message = "Hashtag list must not be greater than 5!")
	private List</*
					 * @Pattern(regexp = "^#[a-zA-Z]{2,16}$",
					 * message="Each Hash Tag must follow the following pattern ^#[a-zA-Z]{2,16}$ ")
					 */ String> hashtags;
